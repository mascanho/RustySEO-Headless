...
/// Renders the external links table, replicating the dashboard's look and feel.
pub fn render_external_links_table(f: &mut Frame, app: &mut App, area: Rect, _title: &str) {
    app.table_rect = Some(area);

    // Initial population if empty
    if app.external_filtered_table_data.is_empty()
        && !app.external_table_data.is_empty()
        && app.external_search_query.is_empty()
    {
        app.apply_external_filter();
    }
    ...
    let selected_idx = app.external_table_state.selected();
    let rows = create_rows(
        &app.external_filtered_table_data,
        selected_idx,
        &app.url_to_status,
    );
    ...
    let pagination_info = format!(
        " Page {} of {} ",
        app.external_current_page + 1,
        total_pages
    );

    let footer_right = Line::from(vec![
        Span::styled(
            format!(" {} Links ", app.external_full_filtered_table_data.len()),
    ...
    f.render_stateful_widget(table, area, &mut app.external_table_state);

    // Render search bar if active
    if app.show_external_search {
        render_search_bar(f, &app.external_search_query, area);
    }
}

fn create_rows<'a>(
    data: &'a Vec<crate::models::ExternalLink>,
    selected_idx: Option<usize>,
    url_to_status: &'a HashMap<String, String>,
) -> Vec<Row<'a>> {
    data.iter()
        .enumerate()
        .map(|(i, link)| {
...
fn calculate_total_pages(app: &App) -> usize {
    let total_items = app.external_full_filtered_table_data.len();
    if total_items == 0 {
        1
    } else {
        (total_items + app.external_page_size - 1) / app.external_page_size
    }
}
...
fn render_search_bar(f: &mut Frame, query: &str, area: Rect) {
    ...
    let block = Block::default()
        ...
        .title(" 🔍 Fuzzy Search (External Links) ");
