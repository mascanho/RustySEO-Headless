...
pub fn render_internal_links_table(f: &mut Frame, app: &mut App, area: Rect, title: &str) {
    ...
    if app.internal_filtered_table_data.is_empty()
        && !app.internal_table_data.is_empty()
        && app.internal_search_query.is_empty()
    {
    ...
    let rows = create_rows(
        &app.internal_filtered_table_data,
        selected_idx,
        &app.url_to_status,
    );
...
fn calculate_total_pages(app: &App) -> usize {
    ...
    if total_items == 0 {
        ...
        (total_items + app.internal_page_size - 1) / app.internal_page_size
    }
}
