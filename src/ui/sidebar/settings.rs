...
pub fn render(f: &mut Frame, app: &mut App, area: Rect, content_block: Block, accent_color: Color, border_color: Color) {
    if let Some(settings) = &app.settings {
        ...
        f.render_widget(content_block.title(Span::styled(" CONFIGURATION ENGINE ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))), main_area);
        ...
        // 1. ENGINE SECTION
        let engine_rows = vec![
            Row::new(vec![Cell::from("  Max Pages"), Cell::from(settings.crawler.max_pages.to_string())]),
            Row::new(vec![Cell::from("  Concurrency"), Cell::from(settings.crawler.concurrency.to_string())]),
            Row::new(vec![Cell::from("  Domain Lock"), Cell::from(if settings.crawler.stay_on_domain { " PROTECTED " } else { " OPEN " }).style(Style::default().fg(Color::Black).bg(if settings.crawler.stay_on_domain { Color::Green } else { Color::Red }))]),
        ];
        let engine_table = Table::new(engine_rows, [Constraint::Percentage(50), Constraint::Percentage(50)])
            .block(Block::default().borders(Borders::ALL).border_style(Style::default().fg(border_color)).title(Span::styled(" 󱐋 ENGINE ", Style::default().fg(Color::Yellow))))
            .style(Style::default().fg(accent_color));
        f.render_widget(engine_table, sections[0]);

        // 2. VIEWPORT SECTION
        let ui_rows = vec![
            Row::new(vec![Cell::from("  Theme"), Cell::from(format!(" {} ", settings.ui.theme.clone())).style(Style::default().fg(Color::Black).bg(Color::Cyan))]),
            Row::new(vec![Cell::from("  Refresh"), Cell::from(format!(" {}ms ", settings.ui.refresh_rate_ms))]),
        ];
        let ui_table = Table::new(ui_rows, [Constraint::Percentage(50), Constraint::Percentage(50)])
            .block(Block::default().borders(Borders::ALL).border_style(Style::default().fg(border_color)).title(Span::styled(" 🎨 VIEWPORT ", Style::default().fg(Color::Magenta))))
            .style(Style::default().fg(accent_color));
        f.render_widget(ui_table, sections[1]);

        // 3. SYSTEM SECTION
        let sys_rows = vec![
            Row::new(vec![Cell::from("  Format"), Cell::from(settings.system.export_format.to_uppercase()).style(Style::default().fg(Color::Green))]),
            Row::new(vec![Cell::from("  Logs"), Cell::from(settings.system.log_level.to_uppercase()).style(Style::default().fg(Color::Yellow))]),
        ];
        let sys_table = Table::new(sys_rows, [Constraint::Percentage(50), Constraint::Percentage(50)])
