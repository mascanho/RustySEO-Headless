...
pub fn render(f: &mut Frame, app: &mut App, area: Rect, content_block: Block, accent_color: Color) {
    ...
    let total_text = vec![Line::from(vec![
        Span::styled(
            format!("  {} ", total_pages),
            Style::default()
                .fg(Color::White)
    ...
    let tech_rows = vec![
        Row::new(vec![
            ...
            Cell::from(format!(
                "{}%",
                if total_pages > 0 {
                    (mobile_yes * 100) / total_pages
                } else {
                    0
                }
            ))
            .style(Style::default().fg(
                if mobile_yes == total_pages && total_pages > 0 {
                    Color::Green
                } else {
                    Color::Yellow
                },
        ...
        Row::new(vec![
            ...
            Cell::from(format!(
                "{}%",
                if total_pages > 0 {
                    (indexable_yes * 100) / total_pages
                } else {
                    0
                }
            ))
            .style(Style::default().fg(
                if indexable_yes == total_pages && total_pages > 0 {
                    Color::Green
                } else {
                    Color::Red
                },
